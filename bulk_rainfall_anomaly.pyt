...
class Tool(object):
    def __init__(self):
        ...
        self.label = "Bulk Rainfall Anomaly"
        self.description = ""
    ...
    def execute(self, parameters, messages):
        ...
        if num_of_years_lst == len(month_year_lst):
            ...
            for k in month_list:
                ...
                newfilename_avg = 'chirps-v2.0.1981-{0}.{1}.monthly.{2}yrs.avg{3}'.format(lst_year, k, years_num, ext)

                if arcpy.Exists(os.path.join(lta_folder, newfilename_avg)):
                    print(newfilename_avg + " exists")
                else:
                    outCellStatistics_avg = CellStatistics(listoffile, "MEAN", "DATA")
                    outCellStatistics_avg.save(os.path.join(lta_folder, newfilename_avg))

            messages.addmessage("Finish Creating Long Term Average Data........")
            messages.addmessage("Start Calculating Rainfall Anomaly........")
            messages.addmessage("Creating folder to save Rainfall Anomaly data........")
            ra_folder = output_folder+ "\\rain_anom_data"
            if not os.path.exists(ra_folder):
                os.mkdir(ra_folder)
